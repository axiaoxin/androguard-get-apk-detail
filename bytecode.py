...
from androconf import (CONF, color_range, enable_colors, error, remove_colors,
                       save_colors, warning)
...
def PrettyShow(m_a, basic_blocks, notes={}):
    ...
    for i in basic_blocks:
        ...
        for ins in instructions:
            ...
            print_fct("%s" % ", ".join(m_a.get_vm().colorize_operands(operands,
                                                                      colors)))
            ...
            if ins == instructions[-1] and i.childs:
                ...
                if (op_value == 0x2b or op_value ==
                    0x2c) and len(i.childs) > 1:  # flake8: noqa
                    ...
                    print_fct(
                        ' '.join("%d:%s" %
                                 ...
                                 for j in range(0, len(i.childs) - 1)) + " ]%s"
                        % normal_color)
                else:
                    if len(i.childs) == 2:
...
#            registers[i] = 0
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockInstruction in DVMBasicMethodBlock.get_instructions(
            ...
            output = ", ".join(mx.get_vm().get_operand_html(i, registers,
                                                            colors, escape,
                                                            textwrap.wrap)
                               for i in operands)
            ...
            if DVMBasicMethodBlock.get_name(
                ...
                for parent in DVMBasicMethodBlock.fathers:
                    ...
                    for idx, child in enumerate(parent[-1].childs):
                        if child[-
                                 1].get_name() == DVMBasicMethodBlock.get_name():
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockChild in DVMBasicMethodBlock.childs:
            ...
            if DVMBasicMethodBlock.get_name() in hooks:
                if DVMBasicMethodBlockChild[-
                                            1] in hooks[DVMBasicMethodBlock.get_name(
                                            )]:
                    ok = True
                    cblock["Edge"].append(
