...
class APK(object):
            ...
            if i == "AndroidManifest.xml":
                ...
                if self.xml[i] != None:
                    self.package = self.xml[i].documentElement.getAttribute(
                        "package")
                    self.androidversion["Code"] = self.xml[i].documentElement.getAttributeNS(
                        NS_ANDROID_URI, "versionCode")
                    self.androidversion["Name"] = self.xml[i].documentElement.getAttributeNS(
    ...
    def parse_cert(self, cert_fname):
        ...
        p7 = M2Crypto.SMIME.PKCS7(
            M2Crypto.m2.pkcs7_read_bio_der(input_bio._ptr()), 1)
    ...
    def parse_icon(self, icon_path=None):
        ...
        icon_paths = [icon.replace("'", '')
                      for icon in parse_icon_rt.split('\n') if icon]
        ...
        for icon in icon_paths:
            ...
            with open(os.path.join(pkg_name_path, icon_name),
                      'w+b') as icon_file:
        ...
        while True and self.axml.is_valid():
            ...
            if _type == START_DOCUMENT:
                ...
                self.buff += u'<' + self.getPrefix(
                    self.axml.getPrefix()) + self.axml.getName() + u'\n'
