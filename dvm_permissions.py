...
DVM_PERMISSIONS = {
    "MANIFEST_PERMISSION": {
        ...
        "SEND_SMS": ["dangerous", "send SMS messages",
                     ...
                     "Malicious applications may cost you money"
                     "by sending messages without your confirmation."],

        "SEND_SMS_NO_CONFIRMATION": ["signatureOrSystem", "send SMS messages",
                                     "send SMS messages via the Messaging app"
                                     "with no user input or confirmation"],

        "RECEIVE_SMS": ["dangerous", "receive SMS",
                        "Allows application to receive and process SMS messages"  # flake8: noqa
                        "Malicious applications may monitor your messages"
                        "or delete them without showing them to you."],

        "RECEIVE_MMS": ["dangerous", "receive MMS",
                        "Allows application to receive and process MMS messages"  #flake8: noqa
                        "Malicious applications may monitor your messages"
                        "or delete them without showing them to you."],

        "RECEIVE_EMERGENCY_BROADCAST": ["signatureOrSystem", "",
                                        "Allows an application to receive"
                                        "emergency cell broadcast messages,"
                                        "to record or display them to the user"
                                        "Reserved for system apps."],

        "READ_CELL_BROADCASTS": ["dangerous", "received cell broadcast messages",
                                 "Allows an application to read previously received"
                                 "cell broadcast messages and to register a content"
                                 "observer to get notifications when a cell broadcast"
                                 "has been received and added to the database. "
                                 "For emergency alerts, the database is updated"
                                 "immediately after the alert dialog and notification"
                                 "sound/vibration/speech are presented.The \"read\""
                                 "column is then updated after the user dismisses the alert."
                                 "This enables supplementary emergency assistance apps"
                                 "to start loading additional emergency information"
                                 "(if Internet access is available) when the alert is first received,"
                                 "and to delay presenting the info to the user"
                                 "until after the initial alert dialog is dismissed."],

        "READ_SMS": ["dangerous", "read SMS or MMS",
                     "Allows application to read SMS messages stored on your phone or SIM card."
                     "Malicious applications may read your confidential messages."],

        "WRITE_SMS": ["dangerous", "edit SMS or MMS",
                      "Allows application to write to SMS messages stored on your phone or SIM card."
                      "Malicious applications may delete your messages."],

        "RECEIVE_WAP_PUSH": ["dangerous", "receive WAP",
                             "Allows application to receive and process WAP messages."
        ...
        "BROADCAST_SMS": ["signature", "send SMS-received broadcast",
                          "Allows an application to broadcast a notification"
                          "that an SMS message has been received."
        ...
        "BROADCAST_WAP_PUSH": ["signature", "send WAP-PUSH-received broadcast",
                               ...
                               "Malicious applications may use this to forge MMS message receipt"
                               "or to replace the content of any web page silently with malicious variants."],

        # SOCIAL_INFO
        "READ_CONTACTS": ["dangerous", "read contact data",
                          "Allows an application to read all of the contact (address) data stored on your phone."
                          "Malicious applications can use this to send your data to other people."],
        "WRITE_CONTACTS": ["dangerous", "write contact data",
                           "Allows an application to modify the contact (address) data stored on your phone."
        ...
        "BIND_APPWIDGET": ["signatureOrSystem", "choose widgets",
                           ...
                           "which widgets can be used by which application."
                           "With this permission, applications can give access"
                           "to personal data to other applications."
                           "Not for use by normal applications."],

        "BIND_KEYGUARD_APPWIDGET": ["signatureOrSystem", "",
                                    "Private permission, to restrict who can bring up a dialog to"
                                    "add a new keyguard widget"],

        # CALENDAR
        "READ_CALENDAR": ["dangerous", "read calendar events",
                          "Allows an application to read all of the calendar events stored on your phone."
                          "Malicious applications can use this to send your calendar events to other people."],
        ...
        "WRITE_CALENDAR": ["dangerous", "add or modify calendar events and send emails to guests",
                           ...
                           "which may send emails to guests. Malicious applications can use this to erase"
                           "or modify your calendar events or to send emails to guests."],
        ...
        "READ_HISTORY_BOOKMARKS": ["dangerous", "read Browser's history and bookmarks",
                                   "Allows the application to read all the URLs that"
                                   "the browser has visited and all of the browser's bookmarks."],

        "WRITE_HISTORY_BOOKMARKS": ["dangerous", "write Browser's history and bookmarks",
                                    "Allows an application to modify the browser's history"
                                    "or bookmarks stored on your phone. Malicious applications"
                                    "can use this to erase or modify your browser's data."],

        # DEVICE_ALARMS
        "SET_ALARM": ["normal", "set alarm in alarm clock",
                      "Allows the application to set an alarm in an installed alarm clock application."
                      "Some alarm clock applications may not implement this feature."],
        ...
        "ACCESS_FINE_LOCATION": ["dangerous", "fine (GPS) location",
                                 "Access fine location sources, such as the Global Positioning System on the phone,"
                                 "where available. Malicious applications can use this to determine where you are"
                                 "and may consume additional battery power."],

        "ACCESS_COARSE_LOCATION": ["dangerous", "coarse (network-based) location",
                                   "Access coarse location sources, such as the mobile network database,"
        ...
        "ACCESS_MOCK_LOCATION": ["dangerous", "mock location sources for testing",
                                 ...
                                 "Malicious applications can use this to override the location and/or status"
                                 "returned by real-location sources such as GPS or Network providers."],

        "ACCESS_LOCATION_EXTRA_COMMANDS": ["normal", "access extra location provider commands",
                                           "Access extra location provider commands."
                                           "Malicious applications could use this to interfere"
        ...
        "INSTALL_LOCATION_PROVIDER": ["signatureOrSystem", "permission to install a location provider",
                                      "Create mock location sources for testing."
                                      "Malicious applications can use this to override the location and/or"
                                      "status returned by real-location sources such as GPS or Network providers,"
        ...
        "NFC": ["dangerous", "control Near-Field Communication",
                "Allows an application to communicate with"
                "Near-Field Communication (NFC) tags, cards and readers."],
        ...
        "BLUETOOTH": ["dangerous", "create Bluetooth connections",
                      "Allows an application to view configuration of the local Bluetooth phone"
                      "and to make and accept connections with paired devices."],

        "BLUETOOTH_ADMIN": ["dangerous", "bluetooth administration",
                            "Allows an application to configure the local Bluetooth phone"
        ...
        "INTERACT_ACROSS_USERS": ["signatureOrSystemOrDevelopment", "",
                                  "Allows an application to call APIs that allow it to do"
                                  "interactions across the users on the device,"
                                  "using singleton services and user-targeted broadcasts."
                                  "This permission is not available to third party applications."],

        "INTERACT_ACROSS_USERS_FULL": ["signature", "",
                                       "Fuller form of INTERACT_ACROSS_USERS that removes restrictions"
                                       "on where broadcasts can be sent and allows other types of interactions."],

        "MANAGE_USERS": ["signatureOrSystem", "",
                         "Allows an application to call APIs that allow it to query and manage users on the device."
                         "This permission is not available to third party applications."],

        "GET_DETAILED_TASKS": ["signature", "", "Allows an application to get full detailed information"
                               "about recently running tasks, with full fidelity to the real state."],

        "START_ANY_ACTIVITY": ["signature", "", "Allows an application to start any activity,"
                               "regardless of permission protection or exported state."],
        ...
        "SET_ANIMATION_SCALE": ["signatureOrSystemOrDevelopment", "modify global animation speed",
                                "Allows an application to change the global animation speed"
                                "(faster or slower animations) at any time."],
        ...
        "SET_PREFERRED_APPLICATIONS": ["signature", "set preferred applications",
                                       "Allows an application to modify your preferred applications."
                                       "This can allow malicious applications to silently change the applications"
                                       "that are run, spoofing your existing applications"
                                       "to collect private data from you."],

        "BROADCAST_STICKY": ["normal", "send sticky broadcast",
                             "Allows an application to send sticky broadcasts,"
                             "which remain after the broadcast ends."
                             "Malicious applications can make the phone slow or unstable"
                             "by causing it to use too much memory."],

        "MOUNT_UNMOUNT_FILESYSTEMS": ["signatureOrSystem", "mount and unmount file systems",
                                      "Allows the application to mount and "
                                      "unmount file systems for removable storage."],
        ...
        "SUBSCRIBED_FEEDS_READ": ["normal", "read subscribed feeds",
                                  "Allows an application to receive details"
                                  "about the currently synced feeds."],
        ...
        "CLEAR_APP_CACHE": ["dangerous", "delete all application cache data",
                            ...
                            "by deleting files in application cache directory."
                            "Access is usually very restricted to system process."],

        "DIAGNOSTIC": ["signature", "read/write to resources owned by diag",
                       "Allows an application to read and write to any resource owned by the diag group;"
                       "for example, files in /dev. This could potentially affect system stability and security."
                       "This should ONLY be used for hardware-specific diagnostics by the manufacturer or operator."],

        "BROADCAST_PACKAGE_REMOVED": ["signature", "send package removed broadcast", "Allows an application to broadcast a notification"
                                      "that an application package has been removed."
                                      "Malicious applications may use this to kill any other application running."],

        "BATTERY_STATS": ["dangerous", "modify battery statistics",
                          "Allows the modification of collected battery statistics."
                          "Not for use by normal applications."],

        "MODIFY_APPWIDGET_BIND_PERMISSIONS": ["signatureOrSystem",
                                              "query/set which applications can bind AppWidgets.",
                                              "Internal permission allowing an application to"
                                              "query/set which applications can bind AppWidgets."],
        ...
        "GLOBAL_SEARCH": ["signatureOrSystem", "", "This permission can be used on"
                          "content providers to allow the global search "
                          "system to access their data. Typically it used when the provider"
                          "has some permissions protecting it"
                          "(which global search would not be expected to hold),"
                          "and added as a read-only permission to the path in the provider where global "
                          "search queries are performed.  This permission can not be held"
                          "by regular applications; it is used by applications to protect themselves"
                          "from everyone else besides global search"],

        "GLOBAL_SEARCH_CONTROL": ["signature", "", "Internal permission protecting access to"
                                  "the global search system: ensures that only the system can"
                                  "access the provider to perform queries (since this otherwise"
                                  "provides unrestricted access to a variety of content providers),"
                                  "and to write the search statistics (to keep applications from"
                                  "gaming the source ranking)."],
        ...
        "WRITE_DREAM_STATE": ["signature", "", "Allows applications to write dream settings,"
                              "and start or stop dreaming."],
        ...
        "AUTHENTICATE_ACCOUNTS": ["dangerous", "act as an account authenticator",
                                  ...
                                  "of the Account Manager, including creating accounts as well as obtaining"
                                  "and setting their passwords."],
        ...
        "CHANGE_WIFI_MULTICAST_STATE": ["dangerous", "allow Wi-Fi Multicast reception",
                                        "Allows an application to receive packets not directly addressed"
                                        "to your device. This can be useful when discovering services offered nearby."
        ...
        "HARDWARE_TEST": ["signature", "test hardware", "Allows the application to control various peripherals"
        ...
        "CAMERA": ["dangerous", "take pictures and videos",
                   ...
                   "This allows the application to collect images that the camera is seeing at any time."],

        # PHONE_CALLS
        "PROCESS_OUTGOING_CALLS": ["dangerous", "intercept outgoing calls",
                                   "Allows application to process outgoing calls and change the number to be dialled."
        ...
        "READ_PHONE_STATE": ["dangerous", "read phone state and identity",
                             ...
                             "An application with this permission can determine the phone number and serial"
                             "number of this phone, whether a call is active, the number that"
        ...
        "DISABLE_KEYGUARD": ["dangerous", "disable key lock",
                             "Allows an application to disable the key lock and any associated password security."
                             "A legitimate example of this is the phone disabling the key lock"
                             "when receiving an incoming phone call, then re-enabling the key lock"
                             "when the call is finished."],

        # APP_INFO
        "GET_TASKS": ["dangerous", "retrieve running applications",
                      "Allows application to retrieve information about currently"
                      "and recently running tasks. May allow malicious applications"
                      "to discover private information about other applications."],

        "REORDER_TASKS": ["normal", "reorder applications running",
                          "Allows an application to move tasks to the foreground and background."
                          "Malicious applications can force themselves to the front without your control."],
        ...
        "RECEIVE_BOOT_COMPLETED": ["normal", "automatically start at boot",
                                   ...
                                   "This can make it take longer to start the phone and allow the application"
                                   "to slow down the overall phone by always running."],

        # DISPLAY
        "SYSTEM_ALERT_WINDOW": ["dangerous", "display system-level alerts",
                                "Allows an application to show system-alert windows."
                                "Malicious applications can take over the entire screen of the phone."],
        ...
        "READ_SYNC_STATS": ["normal", "read sync statistics",
                            "Allows an application to read the sync stats;"
                            "e.g. the history of syncs that have occurred."],

        # DEVELOPMENT_TOOLS
        "WRITE_SECURE_SETTINGS": ["signatureOrSystemOrDevelopment", "modify secure system settings",
                                  "Allows an application to modify the system's secure settings data."
                                  "Not for use by normal applications."],

        "DUMP": ["signatureOrSystemOrDevelopment", "retrieve system internal status",
                 "Allows application to retrieve internal status of the system."
                 "Malicious applications may retrieve a wide variety of private"
                 "and secure information that they should never normally need."],

        "READ_LOGS": ["signatureOrSystemOrDevelopment", "read sensitive log data",
                      "Allows an application to read from the system's various log files."
                      "This allows it to discover general information about what you are doing with the phone,"
        ...
        "SET_DEBUG_APP": ["signatureOrSystemOrDevelopment", "enable application debugging",
                          "Allows an application to turn on debugging for another application."
        ...
        "SET_PROCESS_LIMIT": ["signatureOrSystemOrDevelopment", "limit number of running processes",
                              "Allows an application to control the maximum number of processes that will run."
                              "Never needed for normal applications."],

        "SET_ALWAYS_FINISH": ["signatureOrSystemOrDevelopment", "make all background applications close",
                              "Allows an application to control whether activities are always finished"
                              "as soon as they go to the background. Never needed for normal applications."],

        "SIGNAL_PERSISTENT_PROCESSES": ["signatureOrSystemOrDevelopment", "send Linux signals to applications",
                                        "Allows application to request that the supplied signal be"
                                        "sent to all persistent processes."],

        "ACCESS_ALL_EXTERNAL_STORAGE": ["signature", "", "Allows an application to access all"
        ...
        "FORCE_BACK": ["signature", "force application to close",
                       "Allows an application to force any activity that is"
                       "in the foreground to close and go back."
        ...
        "INTERNAL_SYSTEM_WINDOW": ["signature", "display unauthorised windows",
                                   "Allows the creation of windows that are intended to"
                                   "be used by the internal system user interface."
                                   "Not for use by normal applications."],

        "MANAGE_APP_TOKENS": ["signature", "manage application tokens",
                              "Allows applications to create and manage their own tokens,"
                              "bypassing their normal Z-ordering. "
                              "Should never be needed for normal applications."],

        "FREEZE_SCREEN": ["signature", "", "Allows the application to temporarily freeze"
        ...
        "INJECT_EVENTS": ["signature", "inject user events",
                          ...
                          "Without this permission, you can only deliver events to windows"
                          "in your own process. Very few applications should need to use this permission"],

        "FILTER_EVENTS": ["signature", "", "Allows an application to register an input filter"
                          "which filters the stream of user events (keys, touch, trackball)"
                          "before they are dispatched to any window"],

        "RETRIEVE_WINDOW_INFO": ["signature", "", "Allows an application to retrieve info for"
                                 "a window from the window manager."],

        "TEMPORARY_ENABLE_ACCESSIBILITY": ["signature", "", "Allows an application to temporary enable"
        ...
        "SET_ACTIVITY_WATCHER": ["signature", "monitor and control all application launching",
                                 "Allows an application to monitor and control how the system launches activities."
                                 "Malicious applications may compromise the system completely."
                                 "This permission is needed only for development, never for normal phone usage."],

        "SHUTDOWN": ["signatureOrSystem", "partial shutdown", "Puts the activity manager into a shut-down state."
                     "Does not perform a complete shut down."],
        ...
        "READ_INPUT_STATE": ["signature", "record what you type and actions that you take",
                             "Allows applications to watch the keys that you press even"
                             "when interacting with another application (such as entering a password)."
                             "Should never be needed for normal applications."],
        ...
        "BIND_TEXT_SERVICE": ["signature", "", "Must be required by a TextService (e.g. SpellCheckerService)"
                              "to ensure that only the system can bind to it."],

        "BIND_VPN_SERVICE": ["signature", "", "Must be required by an {@link android.net.VpnService},"
                             "to ensure that only the system can bind to it."],
        ...
        "BIND_DEVICE_ADMIN": ["signature", "interact with device admin",
                              "Allows the holder to send intents to a device administrator."
                              "Should never be needed for normal applications."],

        "SET_ORIENTATION": ["signature", "change screen orientation",
                            "Allows an application to change the rotation of the screen at any time."
                            "Should never be needed for normal applications."],
        ...
        "INSTALL_PACKAGES": ["signatureOrSystem", "directly install applications",
                             "Allows an application to install new or updated Android packages."
                             "Malicious applications can use this to add new applications"
                             "with arbitrarily powerful permissions."],
        ...
        "DELETE_PACKAGES": ["signatureOrSystem", "delete applications",
                            "Allows an application to delete Android packages."
                            "Malicious applications can use this to delete important applications."],

        "MOVE_PACKAGE": ["signatureOrSystem", "Move application resources",
                         "Allows an application to move application resources"
                         "from internal to external media and vice versa."],

        "CHANGE_COMPONENT_ENABLED_STATE": ["signatureOrSystem", "enable or disable application components",
                                           "Allows an application to change whether or not a component"
                                           "of another application is enabled. Malicious applications can use this"
                                           "to disable important phone capabilities."
                                           "It is important to be careful with permission, "
        ...
        "CONTROL_WIFI_DISPLAY": ["signature", "", "Allows an application to control low-level features"
                                 "of Wifi displays such as opening an RTSP socket."
                                 "This permission should only be used by the display manager."],

        "BRICK": ["signature", "permanently disable phone",
                  "Allows the application to disable the entire phone permanently."
        ...
        "FACTORY_TEST": ["signature", "run in factory test mode",
                         ...
                         "allowing complete access to the phone hardware."
                         "Only available when a phone is running in manufacturer test mode."],
        ...
        "MASTER_CLEAR": ["signatureOrSystem", "reset system to factory defaults",
                         ...
                         "erasing all data, configuration and installed applications."],

        "CALL_PRIVILEGED": ["signatureOrSystem", "directly call any phone numbers",
                            "Allows the application to call any phone number,"
                            "including emergency numbers, without your intervention."
                            "Malicious applications may place unnecessary"
        ...
        "PERFORM_CDMA_PROVISIONING": ["signatureOrSystem", "directly start CDMA phone setup",
                                      ...
                                      "Malicious applications may start CDMA provisioning unnecessarily"],
        ...
        "ACCESS_CHECKIN_PROPERTIES": ["signatureOrSystem", "access check-in properties",
                                      "Allows read/write access to properties uploaded by the check-in service."
                                      "Not for use by normal applications."],

        "PACKAGE_USAGE_STATS": ["signatureOrSystem", "update component usage statistics",
                                "Allows the modification of collected component usage statistics."
                                "Not for use by normal applications."],

        "BACKUP": ["signatureOrSystem", "control system back up and restore",
                   "Allows the application to control the system's back-up and restore mechanism."
                   "Not for use by normal applications."],

        "CONFIRM_FULL_BACKUP": ["signature", "", "Allows a package to launch the secure full-backup confirmation UI."
                                "ONLY the system process may hold this permission."],

        "BIND_REMOTEVIEWS": ["signatureOrSystem", "", "Must be required by a {@link android.widget.RemoteViewsService},"
                             "to ensure that only the system can bind to it."],
        ...
        "COPY_PROTECTED_DATA": ["signature", "Allows to invoke default container service to copy content."
                                "Not for use by normal applications.",
                                "Allows to invoke default container service to copy content."
                                "Not for use by normal applications."],
        ...
        "READ_NETWORK_USAGE_HISTORY": ["signatureOrSystem", "read historical network usage for"
                                       "specific networks and applications.",
                                       "Allows an application to read historical network usage"
                                       "for specific networks and applications."],

        "MANAGE_NETWORK_POLICY": ["signature", "manage network policies and to define application-specific rules.",
                                  "Allows an application to manage network policies and to"
        ...
        "PACKAGE_VERIFICATION_AGENT": ["signatureOrSystem", "Package verifier needs to have this permission"
                                       "before the PackageManager will trust it to verify packages.",
                                       "Package verifier needs to have this permission before the "
                                       "PackageManager will trust it to verify packages."],

        "BIND_PACKAGE_VERIFIER": ["signature", "", "Must be required by package verifier receiver,"
                                  "to ensure that only the system can interact with it.."],

        "SERIAL_PORT": ["signature", "", "Allows applications to access serial ports via the SerialManager."],

        "ACCESS_CONTENT_PROVIDERS_EXTERNALLY": ["signature", "", "Allows the holder to access content"
                                                "providers from outside an ApplicationThread."
                                                "This permission is enforced by the ActivityManagerService on"
                                                "the corresponding APIs,in particular"
                                                "ActivityManagerService#getContentProviderExternal(String)"
                                                "and ActivityManagerService#removeContentProviderExternal(String)."],

        "UPDATE_LOCK": ["signatureOrSystem", "", "Allows an application to hold an UpdateLock,"
                        "recommending that a headless OTA reboot *not* occur while the lock is held"],

        "WRITE_GSERVICES": ["signatureOrSystem", "modify the Google services map",
                            "Allows an application to modify the Google services map."
                            "Not for use by normal applications."],
    ...
    "MANIFEST_PERMISSION_GROUP":
    {
        ...
        "AFFECTS_BATTERY": "Used for permissions that provide direct access to the"
                            "hardware on the device that has an effect on battery life."
                            "This includes vibrator, flashlight,  etc.",
        ...
        "CAMERA": "Used for permissions that are associated with accessing camera or"
                  "capturing images/video from the device.",
        "COST_MONEY": "Used for permissions that can be used to make the user spend money"
        ...
        "MESSAGES": "Used for permissions that allow an application to send messages on behalf of the user"
        ...
        "PERSONAL_INFO": "Used for permissions that provide access to the user's private data,"
                         "such as contacts, calendar events, e-mail messages, etc.",
        "PHONE_CALLS": "Used for permissions that are associated with accessing and modifyign telephony state:"
                       "intercepting outgoing calls, reading and modifying the phone state.",
        "STORAGE": "Group of permissions that are related to SD card access.",
        "SOCIAL_INFO": "Used for permissions that provide access to the user's social connections,"
                       "such as contacts, call logs, social stream, etc. "
                       "This includes both reading and writing of this data"
                       "(which should generally be expressed as two distinct permissions)",
